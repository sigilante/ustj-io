...
\fancyfoot[L]{{\footnotesize
              ...
              {\it Urbit Systems Technical Journal} III:2 (2026):  45–58. \\
              ~ \\
...
\begin{abstract}
  Urbit's input and output system is structured as explicit state
  machines, with a small amount of monadic I/O at the top level.
  This is a deliberate design choice, and we explore the design space.
\end{abstract}
...
\item
  ...
  cleanly upgrade by killing the thread and restarting it, or
  letting it resolve to a defined ``upgradable state'' periodically,
  usually at the top of a ``main loop''. In the latter case, then you are in
  fact structuring it as a state machine, so other advantages of
...
This is convenient and eliminates many long-tail bugs, but it also
means we need to code for permanence, and that's much easier when you
structure code with explicit state.
...
respond during the \texttt{callAgent()} function. If another request comes in,
...
I won't claim this is easier to write than a monadic
...
Monadic and state machine pseudocode are given in Listing \ref{lst:monadic} and \ref{lst:state-machine}.
...
\begin{lstlisting}[style=listingblock,
                   float=b!,
                   label={lst:monadic},
                   caption={Monadic I/O example of fetching top stories and comments.},]
...
\begin{lstlisting}[style=listingblock,
                   ...
                   caption={State machine example of fetching top stories and comments.},]
state :: Initial
       | FetchingTop
...
\begin{lstlisting}[style=listingblock,
                   float=p!,
                   caption={Jael's state machine},
                   label={lst:jael-state-machine}]
    ...
    TimerFired →
      ...
      [State txs newId,
       append(effects,
              ...
              StartTimer (now + five minutes))]
    ThreadFinished Failure → [State block txs Null,
                              Null]
...
From Jael's perspective, it looks something like Listing~\ref{lst:jael-state-machine}.  In other words, it's just a single I/O event
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item  There are three styles of I/O: imperative, monadic, and explicit state machines.
  \item  Urbit supports explicit state machines (agents) and monadic I/O
  (threads).
  \item  Explicit state machines are preferred for permanent, robust,
  concurrent code.
  \item  Monadic I/O is preferred for short-lived code with complex I/O.
  \item  If a state machine needs to perform complex I/O, it should encapsulate
